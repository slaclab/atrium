...
class USR_BUFF_TESTER:
    ...
    def core_test_user_buffers(self, signal_pvs, variable_type_suffixes, usr_buff_idx_range, usr_buff_fixed_rates, samples, bsss):
        ...
        settings.bsss_usr_buff_acq = bsss 
        ...
        self.reset_user_buffer(settings.bsa_usr_buff_rate_mode_pv, settings.bsa_usr_buff_fixed_rate_pv, settings.bsa_usr_buff_ac_rate_pv, settings.bsa_usr_buff_control_pv)     
        self.user_buffer_pid_pv_data = {}
        self.user_buffer_pv_data = {}
        self.user_buffer_single_data = {}
    ...
    def reset_user_buffer(self, rate_mode_pv, fixed_rate_pv, ac_rate_pv, control_pv):
            ...
            caput(fixed_rate_pv, list(settings.bsa_usr_buff_fixed_rates.keys())[0], wait=True)
            caput(ac_rate_pv, list(settings.bsa_usr_buff_ac_rates.keys())[0], wait=True)
    ...
    def get_pvs_data_pair(self, pvlist):
        ...
        while True:
            ...
            for pv_name in pvlist:
                ...
                elif np.array(self.user_buffer_pv_data[pv_name].signal_data1).size < settings.bsa_usr_buff_samples:
                    all_pvs_data_acquired = False
            ...
            for pv_name in pvlist:
                if settings.bsss_usr_buff_acq:
                    if np.array(self.user_buffer_pv_data[pv_name].signal_data2).size != 1:
                        all_pvs_data_acquired = False
                if np.array(self.user_buffer_pv_data[pv_name].signal_data2).size < settings.bsa_usr_buff_samples:
                    all_pvs_data_acquired = False
            if all_pvs_data_acquired:
                break
    ...
    def test_bsa_user_buffers(self):
        ...
        settings.bsa_usr_buff_idx_range, settings.bsa_usr_buff_fixed_rates, samples=settings.bsa_usr_buff_samples_num_elem_test, bsss=False) 
    ...
    def test_bsss_user_buffers(self):
        ...
        settings.bsss_usr_buff_fixed_rates, samples=1, bsss=True)